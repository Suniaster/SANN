...
fn performance_compare(c: &mut Criterion){
    ...
    group.bench_with_input(BenchmarkId::new("old", 6),&LOOP_COUNT,  |b, i| b.iter(|| xor_net.activate(input)));
    ...
    group.bench_with_input(BenchmarkId::new("new", 6),&LOOP_COUNT, |b,i| b.iter(|| ann.activate(&input)));
    group.finish();
}
