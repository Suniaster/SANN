...
impl NeuralNet {
        ...
        for layer in self.layers.iter_mut(){
            for neuron_i in 0..layer.weights_matrix.shape()[0] { 
                for neuron_weight_index in 0..layer.weights_matrix.shape()[1]{
                    ...
                    layer.weights_matrix[[neuron_i,neuron_weight_index]] += 
                        learning_rate * layer.deltas[neuron_i]  * last_layer_out[neuron_weight_index];
                }
            }

            last_layer_out = &layer.last_output;
        } 
        return &self.layers.iter().rev().next().expect("No layer").last_output;
    }
    ...
    pub fn backward(&mut self, expected: &Array1<f64>){
        ...
        let errors:Array1<f64> = output_layer.last_output.iter().zip(expected.iter())
            .map(| (w, expected) | -> f64 { NeuralNet::calculate_loss_derivative(expected, w)}).collect();

        output_layer.deltas = errors.iter().zip(output_layer.last_output.iter())
            .map( |(err, out)| -> f64 {(*err) * (output_layer.activation.d)(out) }).collect();
        ...
        for layer in iterator {
            ...
            for neuron_i in 0..layer.weights_matrix.shape()[0] {
                ...
                for prev_layer_neuron_index in 0..prev_layer.weights_matrix.shape()[0]{
                    let weight = prev_layer.weights_matrix.get((prev_layer_neuron_index, neuron_i))
                        .expect("Incorrect acess on prev-layer backpropagation");
                    let delta = prev_layer.deltas.get(prev_layer_neuron_index)
            ...
            layer.deltas = errors.iter().zip(layer.last_output.iter())
                .map( |(err, out)| -> f64 {(*err) * (layer.activation.d)(out) }).collect();
            prev_layer = layer;
